...
## 대규모 페이지 전략 (Figma 등 요소가 많을 때)

> **페이지 전체를 한 번에 처리하지 마세요.** 요소가 많으면 컨텍스트가 넘쳐서 추론 모드로 빠지고, Atomic CSS 대신 일반 CSS 클래스를 사용하는 치명적 오류가 발생합니다.

### 반드시 섹션 분할

1. **먼저 페이지 구조를 파악** — 디자인을 논리적 섹션으로 나눕니다:
   - Header/Nav, Hero, 제품 그리드, 기능 소개, CTA, Footer 등
2. **섹션별로 독립 처리** — 각 섹션마다 아래 3단계 워크플로우를 개별 적용
3. **한 섹션이 끝나면 다음 섹션으로** — 절대 여러 섹션을 동시에 처리하지 않음
4. **마지막에 조립** — 모든 섹션을 하나의 페이지로 합침

### 컨텍스트 초과 감지 — 이런 상황이면 분할 필수
- Figma에서 가져온 요소가 20개 이상
- 전체 페이지 또는 랜딩 페이지를 요청받았을 때
- 여러 독립적인 UI 블록이 포함된 디자인
...
### Figma MCP가 코드를 반환하는 경우 (우선 경로)

Figma MCP(`get_design_context` 등)가 HTML+CSS 코드를 반환하면, 이 코드에서 CSS 속성을 직접 추출할 수 있으므로 더 정확합니다:
...
> Figma MCP가 코드를 반환하지 않고 메타데이터만 반환하는 경우, 아래 기본 3단계 워크플로우로 폴백합니다.
...
> **대규모 페이지의 경우, 각 섹션마다 이 3단계를 반복합니다.**
